...
class ChatService
{
     ...
     */
    public function initiateChat($consumerId, $providerId, $question = null, bool $isPackageChat = false)
    {
        return DB::transaction(function () use ($consumerId, $providerId, $question, $isPackageChat) {
            try {
    ...
    public function acceptChat($sessionId, $providerId)
    {
        return DB::transaction(function () use ($sessionId, $providerId) {
            try {
                ...
                $systemMessage = null;
                // Format & store consumer birth details ONLY if not already sent to this astrologer in the last 24 hours
                if ($this->shouldSendBirthDetails($session->consumer_id, $session->provider_id)) {
                ...
                if ($isPrepaid) {
                    $subSession = \App\Models\PackageSubSession::where('chat_session_id', $sessionId)->first();
     ...
     */
    private function shouldSendBirthDetails($consumerId, $providerId): bool
    {
        $recentSessionIds = \App\Models\ChatSession::where(function ($q) use ($consumerId, $providerId) {
        ...
        if ($recentSessionIds->isNotEmpty()) {
            $alreadySent = \App\Models\Message::whereIn('chat_session_id', $recentSessionIds)
    ...
    /**
     * Reject an initiated chat request.
     */
